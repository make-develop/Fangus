...
class DBOperations{
   ...
   private $user = 'makedeve_mike';
   private $db = 'makedeve_fangus';
   private $pass = 'Proyectox1116';
   private $conn;
...
public function __construct() {
 ...
 public function insertData($name, $lastname, $dateb, $email, $profession, $tlf, $password){
  ...
  $sql = 'INSERT INTO LoginInstructor SET unique_id =:unique_id,email =:email,encrypted_password =:encrypted_password,salt =:salt,created_at = NOW()';
    
    $sql2 = 'INSERT INTO Instructor SET unique_id =:unique_id,name =:name,lastname =:lastname,dateb =:dateb,email =:email,profession =:profession,tlf =:tlf';

  $query = $this ->conn ->prepare($sql);
  $query->execute(array('unique_id' => $unique_id, ':email' => $email,':encrypted_password' => $encrypted_password, ':salt' => $salt));

 $query2 = $this ->conn ->prepare($sql2);
  $query2->execute(array('unique_id' => $unique_id, ':name' => $name, ':lastname' => $lastname, ':dateb' => $dateb,':email' => $email, ':profession' => $profession,':tlf' => $tlf));
    if ($query && $query2) {
 ...
 public function checkLogin($email, $password) {
    ...
    $sql = 'SELECT * FROM LoginInstructor WHERE email = :email';
    $query = $this -> conn -> prepare($sql);
 ...
 public function changePassword($email, $password){
    ...
    $sql = 'UPDATE LoginInstructor SET encrypted_password = :encrypted_password, salt = :salt WHERE email = :email';
    $query = $this -> conn -> prepare($sql);
 ...
 public function checkUserExist($email){
    ...
    $sql = 'SELECT COUNT(*) from LoginInstructor  WHERE email =:email';
    $query = $this -> conn -> prepare($sql);
    $query -> execute(array('email' => $email));
